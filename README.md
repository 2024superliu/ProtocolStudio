...
分隔符模式和固定长度模式都有空闲时间兜底。例如设置分隔符 `0D 0A`，但本次数据中没有出现该字节序列，缓存会在达到“空闲间隔”后自动输出，不会一直隐藏。
...
插件功能用于把原始字节转换成业务字段。程序内置“Modbus RTU 解析器”和“通用字节解析”，启动时默认选择 Modbus RTU 解析器。Modbus 解析器支持常用的 01、02、03、04、05、06、0F、10、16、17 功能码以及异常响应，可以显示从站地址、功能码、寄存器或线圈参数及 CRC 状态；通用解析器则用于查看长度、HEX、ASCII 和首尾字节。其他设备协议仍可通过外部 DLL 扩展。

截图右侧选择的是“通用字节解析”；当前版本也可以在同一个下拉框中切换到默认的“Modbus RTU 解析器”。例如输入 `01 03 00 00 00 02 C4 0B`，可以解析为“从站 1、读取保持寄存器、起始地址 0、数量 2、CRC 正确”。将自定义 DLL 放入插件目录后，点击“重新加载插件”，扫描结果会直接显示在解析器下方；加载成功的解析器会出现在下拉列表中。“打开插件目录”用于快速定位程序实际扫描的位置，“实时解析”则决定接收到的新帧是否自动进入所选解析器。
...
1. 点击“选择插件目录”；
2. 选择一个已经存在并存放插件 DLL 的文件夹；
3. 选择完成后程序会立即扫描，也可以在替换 DLL 后点击“重新加载插件”；
4. 顶部状态会显示扫描的 DLL 数量、成功加载数量或具体错误；
5. 成功后，插件会出现在解析器下拉框中。

程序启动时只加载内置解析器，不会自动创建 `Plugins` 目录。外部插件采用内存加载方式，不会长期锁住所选目录中的原始 DLL；运行期间可以替换插件文件，再次点击重新加载。
...
插件开发的简要说明保存在源码目录的 `Plugins/README.md`，不会在程序启动或构建输出目录中自动创建插件文件夹。
...
- **发送文件**：只发送原文件字节，不添加文件名、长度、校验值或自定义文件头，兼容普通串口助手、网络助手和真实设备。

发送框不会自动追加 CRC。当前版本按照中继调试工具的定位，将 CRC 作为接收验证和协议诊断功能；如果设备要求发送报文本身包含 CRC，应在发送内容或报文模板中提供完整字节。

文件发送不会经过文本分帧、CRC 追加或自定义文件头逻辑，线路上的字节与源文件完全一致。发送文件期间会暂时停止透明转发，防止其他转发数据插入文件正文。
...
3. 发送端只输出原文件字节；外部助手发送的裸文件同样不携带文件名和长度，接收端通过接收会话或空闲时间判断结束；
